...
const (
	MsgToServer   = int16(689)
	MsgFromServer = int16(690)
	MsgEnding     = int8(0)
)
...
const (
	...
	MsgTypeLoginRes = "loginres" // 登录响应消息
	...
	MsgTypeOnlineGift = "onlinegift" //领取在线鱼丸暴击消息
	...
	MsgTypeUserEnter = "uenter" // 特殊用户进房通知消息
	...
	MsgTypeBcBuyDeserve = "bc_buy_deserve" // 用户赠送酬勤通知消息
	...
	MsgTypeRankList = "ranklist" // 广播排行榜消息

	MsgTypeSSD = "ssd" // 超级弹幕消息(如，火箭弹幕)
	// 超级弹幕主要部分应包含的字段如下:
	// 字段说明
	...
	MsgTypeRankup = "rankup" // 房间内top10变化消息
...
type Message struct {
	body           map[string]interface{} // 消息正文map
	headerType     int16                  // 消息类型，2字节，689为客户端发给服务器
...
// NewMessage(nil, MESSAGE_TO_SERVER)
// NewMessage(newMap, MESSAGE_FROM_SERVER)
func NewMessage(body map[string]interface{}, mtype int16) *Message {
	if body == nil {
...
func (msg *Message) Encode() []byte {
	...
	_ = binary.Write(buffer, binary.LittleEndian, int32(length))
	_ = binary.Write(buffer, binary.LittleEndian, int32(length))
	_ = binary.Write(buffer, binary.LittleEndian, int16(msg.headerType))
	_ = binary.Write(buffer, binary.LittleEndian, int8(msg.headerSecret))
	_ = binary.Write(buffer, binary.LittleEndian, int8(msg.headerReserved))
	_ = binary.Write(buffer, binary.LittleEndian, []byte(content))
	_ = binary.Write(buffer, binary.LittleEndian, MsgEnding)
	return buffer.Bytes()
}
